...
- 한글 COM 연결 확인
- `bufs/보고서 본문 서식.hwpx` 스타일 이름 읽기
- 목록에서 선택한 스타일을 현재 한글 문서의 현재 문단/선택 영역에 적용
- `bufs/표지.hwpx` 자리표시자 확인
...
1. `서식 테스트 문서 열기`를 누르면 `bufs/보고서 본문 서식.hwpx`의 복사본을 `bufs/test-output/`에 만들고 연다.
...
- `bufs/보고서 본문 서식.hwpx`에서 시작한 문서나 스타일 세트를 이미 가져온 문서에서 먼저 테스트한다.
...
- 현재 `bufs/보고서 본문 서식.hwpx`에서 이 기준으로 읽히는 스타일 수는 `16`개다.

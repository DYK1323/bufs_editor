...
![대상 문서 확인 예시](assets/screenshots/3.1_대상문서확인.png)
...
| 표 자동화 | 이미 만들어진 표에 기본 문단 스타일, 셀/표 여백, 기본 표 스타일, 제목셀 설정, 너비 맞춤을 한 번에 적용합니다. |
| `셀여백` | 선택한 셀 안쪽 여백을 기본값으로 맞춥니다. 표 안 글자가 셀 경계에 너무 붙어 보일 때 사용합니다. |
...
| `표 설정` | 기본 표 설정, 제목셀 설정, 색상/선 프리셋, 캡션 인식 규칙 같은 표 관련 설정을 엽니다. |
...
자주 쓰는 표 모양이 있다면 이 설정을 먼저 맞춰 두는 편이 좋습니다.
...
증빙자료 만들기는 PDF나 이미지 파일을 증빙 양식에 넣습니다. PDF는 이미지로 변환한 뒤 삽입하고, 이미지 파일은 변환 과정 없이 바로 삽입합니다.
...
이미지 여러 개를 한 번에 선택하면 선택한 순서대로 한 증빙 양식에 들어갑니다. 증빙 제목은 첫 번째 이미지 파일 이름을 기준으로 자동 입력됩니다.
...
보고양식, 대제목 번호박스, 증빙자료는 현재 커서 위치에 새 내용을 넣습니다. 커서가 입력 가능한 위치에 있는지 먼저 확인합니다.
